<template>
	<img :src="variety.icon" :alt="variety.name" v-if="varietyName" />
</template>
	...
	export default {
		...
		methods: {
			SelectVariety(variety) {
				const VARIETIES = {
					mega: { name: "Mega", icon: KeyStone },
					gmax: { name: "Gigamax", icon: Giga },
		...
		created() {
			this.variety = this.SelectVariety(this.varietyName);
		},
	};
</script>
